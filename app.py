...
from services.vuln_service import get_vulnerabilities
from services.breach_service import get_breach_info
...
def detect_protocol(hostname: str, timeout: int = 4) -> str:
    """Detect whether a host supports HTTPS (tries port 443 TCP connect).
    Returns 'https' if port 443 is reachable, otherwise 'http'.
    """
    try:
        with _socket.create_connection((hostname, 443), timeout=timeout):
            return 'https'
    except Exception:
        return 'http'
...
def api_scan():
    ...
    try:
        ...
        # ── Strip any existing scheme so user can enter domain-only ──────────
        # Remove http:// or https:// prefix if present so we can auto-detect.
        stripped = re.sub(r'^https?://', '', raw_input, flags=re.IGNORECASE).rstrip('/')

        # Extract hostname for protocol detection (strip path/port if any)
        hostname_for_detect = stripped.split('/')[0].split(':')[0]

        # Auto-detect protocol by attempting TCP connect on port 443
        detected_protocol = detect_protocol(hostname_for_detect)

        # Build the canonical URL with detected protocol
        url = f'{detected_protocol}://{stripped}'
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            done, not_done = wait(futures.keys(), timeout=min(config.REQUEST_TIMEOUT * 2, 14))
        ...
        # ── Run Vulnerability & Breach Lookups (after tech detection) ────────
        detected_techs = results.get('technologies', {}).get('technologies', [])
        dns_data_for_breach = results.get('dns', {})

        with ThreadPoolExecutor(max_workers=2) as executor2:
            future_vuln   = executor2.submit(get_vulnerabilities, detected_techs)
            future_breach = executor2.submit(get_breach_info, domain)

            done2, _ = wait([future_vuln, future_breach], timeout=min(config.REQUEST_TIMEOUT, 10))

        try:
            results['vulnerabilities'] = future_vuln.result() if future_vuln in done2 else {'available': False, 'error': 'Timed out'}
        except Exception:
            results['vulnerabilities'] = {'available': False, 'error': 'Vulnerability lookup failed'}

        try:
            results['breach_info'] = future_breach.result() if future_breach in done2 else {'available': False, 'error': 'Timed out'}
        except Exception:
            results['breach_info'] = {'available': False, 'error': 'Breach lookup failed'}
        ...
        # ── Derive Website Information fields ────────────────────────────────
        dns_res  = results.get('dns', {})
        ssl_res  = results.get('ssl', {})
        tech_res = results.get('technologies', {})

        # Primary IP
        ip_address = None
        if dns_res.get('ip_addresses'):
            ip_address = dns_res['ip_addresses'][0]

        # Server (first Web Server tech detected)
        server_tech = next(
            (t['name'] for t in tech_res.get('technologies', [])
             if t.get('category') == 'Web Server'),
            None
        )

        # SSL status label
        ssl_status_raw   = ssl_res.get('status', 'no_ssl')
        ssl_status_label = ssl_status_raw.replace('_', ' ').title() if ssl_status_raw else 'Unknown'

        website_info = {
            'domain':     domain,
            'ip_address': ip_address or 'Not Available',
            'server':     server_tech or 'Not Detected',
        ...
        response = {
            ...
            'vulnerabilities': results.get('vulnerabilities', {'available': False}),
            'breach_info':     results.get('breach_info',     {'available': False}),
        }
