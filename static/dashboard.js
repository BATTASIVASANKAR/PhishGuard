...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle Top "Scan Another URL" button
    const scanAnotherTopBtn = document.getElementById('scan-another-top-btn');
    if (scanAnotherTopBtn) {
        scanAnotherTopBtn.addEventListener('click', () => {
...
function renderDashboard(data) {
    ...
    renderWebsiteInfo(data);

    // 9. Real-Time Vulnerabilities
    renderVulnerabilities(data.vulnerabilities);

    // 10. Data Breach Information
    renderBreachInfo(data.breach_info);
}
...
function renderWebsiteInfo(data) {
    const table = document.getElementById('website-info-table');
    if (!table) return;
    table.innerHTML = '';

    const wi = data.website_info || {};

    const protocolColor = (wi.protocol === 'HTTPS') ? 'var(--safe-color)' : 'var(--warning-color)';
    const protocolIcon  = (wi.protocol === 'HTTPS') ? '🔒' : '🔓';

    let sslColor = 'var(--safe-color)';
    const sslStatus = (wi.ssl_status || '').toLowerCase();
    if (sslStatus.includes('expired') || sslStatus.includes('no ssl')) sslColor = 'var(--danger-color)';
    else if (sslStatus.includes('self') || sslStatus.includes('expiring') || sslStatus.includes('invalid')) sslColor = 'var(--warning-color)';

    const rows = [
        ['Domain Name', wi.domain || data.domain || 'Unknown'],
        ['IP Address',  wi.ip_address || 'Not Available'],
        ['Server',      wi.server || 'Not Detected'],
        ['SSL Certificate Status', `<span style="color:${sslColor}; font-weight:bold">${wi.ssl_status || 'Unknown'}</span>`],
        ['Protocol',   `<span style="color:${protocolColor}; font-weight:bold">${protocolIcon} ${wi.protocol || 'Unknown'}</span>`],
    ];

    rows.forEach(([label, value]) => {
        table.innerHTML += `<tr><td class="label">${label}</td><td class="value">${value}</td></tr>`;
    });
}

// ── Real-Time Vulnerabilities ────────────────────────────────────────────────

function renderVulnerabilities(vulnData) {
    const container = document.getElementById('vuln-content');
    if (!container) return;
    container.innerHTML = '';

    if (!vulnData || !vulnData.available) {
        const msg = vulnData && vulnData.error
            ? vulnData.error
            : 'No vulnerability data available for detected technologies.';
        container.innerHTML = `<div style="color:var(--text-muted); padding:0.5rem;">${msg}</div>`;
        return;
    }

    if (!vulnData.results || vulnData.results.length === 0) {
        container.innerHTML = `<div style="color:var(--safe-color); padding:0.5rem;">✓ No recent CVEs found for detected technologies.</div>`;
        return;
    }

    let html = '';
    vulnData.results.forEach(techResult => {
        html += `
            <div style="margin-bottom:1.5rem;">
                <div style="font-weight:600; font-size:0.95rem; color:var(--text-primary); margin-bottom:0.6rem; padding-bottom:0.4rem; border-bottom:1px solid var(--border-color);">
                    💻 ${techResult.tech_name} — ${techResult.cves.length} Recent CVE(s)
                </div>`;

        techResult.cves.forEach(cve => {
            const cvssVal = parseFloat(cve.cvss);
            let cvssColor = 'var(--text-muted)';
            if (!isNaN(cvssVal)) {
                if (cvssVal >= 9.0)      cvssColor = 'var(--danger-color)';
                else if (cvssVal >= 7.0) cvssColor = '#f97316';  /* orange */
                else if (cvssVal >= 4.0) cvssColor = 'var(--warning-color)';
                else                     cvssColor = 'var(--safe-color)';
            }

            html += `
                <div style="padding:0.75rem; margin-bottom:0.5rem; border-radius:8px; background:var(--bg-secondary); border:1px solid var(--border-color);">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.4rem; flex-wrap:wrap; gap:0.4rem;">
                        <span style="font-family:monospace; font-size:0.82rem; color:var(--accent-primary); font-weight:600;">${cve.id}</span>
                        <div style="display:flex; gap:0.75rem; font-size:0.78rem;">
                            <span style="color:${cvssColor}; font-weight:bold;">CVSS: ${cve.cvss !== 'N/A' ? cve.cvss : 'N/A'}</span>
                            <span style="color:var(--text-muted);">📅 ${cve.published}</span>
                        </div>
                    </div>
                    <div style="font-size:0.83rem; color:var(--text-secondary); line-height:1.5;">${cve.summary}</div>
                </div>`;
        });
...
function renderBreachInfo(breachData) {
    const container = document.getElementById('breach-content');
    if (!container) return;
    container.innerHTML = '';

    if (!breachData || !breachData.available) {
        const msg = breachData && breachData.error
            ? breachData.error
    ...
    if (!breachData.breached) {
        container.innerHTML = `<div style="color:var(--safe-color); padding:0.5rem;">✓ No publicly known data breaches found for this domain.</div>`;
        return;
    }

    const count = breachData.breach_count;
    let html = `
        <div style="margin-bottom:1rem; padding:0.75rem; border-radius:8px; background:#ef444415; border:1px solid #ef444430;">
            <span style="color:var(--danger-color); font-weight:bold; font-size:1rem;">
                ⚠ ${count} data breach${count !== 1 ? 'es' : ''} found for this domain
            </span>
        </div>`;

    breachData.breaches.forEach(b => {
        const pwnFormatted = b.pwn_count
            ? b.pwn_count.toLocaleString()
            : 'Unknown';

        const verifiedBadge = b.is_verified
            ? `<span style="background:#22c55e20;color:var(--safe-color);padding:2px 7px;border-radius:4px;font-size:0.73rem;border:1px solid #22c55e40;">✓ Verified</span>`
            : `<span style="background:#f59e0b20;color:var(--warning-color);padding:2px 7px;border-radius:4px;font-size:0.73rem;border:1px solid #f59e0b40;">Unverified</span>`;

        const sensitiveTag = b.is_sensitive
            ? `<span style="background:#ef444420;color:var(--danger-color);padding:2px 7px;border-radius:4px;font-size:0.73rem;border:1px solid #ef444440; margin-left:4px;">Sensitive</span>`
            : '';

        const dataClasses = b.data_classes && b.data_classes.length
            ? b.data_classes.slice(0, 8).map(dc =>
                `<span style="display:inline-block;margin:2px;padding:2px 8px;border-radius:12px;background:var(--bg-secondary);border:1px solid var(--border-color);font-size:0.73rem;color:var(--text-secondary);">${dc}</span>`
              ).join('') + (b.data_classes.length > 8 ? `<span style="font-size:0.73rem;color:var(--text-muted);margin-left:4px;">+${b.data_classes.length - 8} more</span>` : '')
            : '<span style="color:var(--text-muted);font-size:0.8rem;">Not specified</span>';

        html += `
            <div style="padding:1rem; margin-bottom:0.75rem; border-radius:10px; background:var(--bg-secondary); border:1px solid var(--border-color);">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:0.5rem; margin-bottom:0.6rem;">
                    <div style="font-weight:bold; font-size:0.95rem; color:var(--text-primary);">
                        🔓 ${b.title}
                    </div>
                    <div style="display:flex; align-items:center; gap:0.3rem; flex-wrap:wrap;">
                        ${verifiedBadge}${sensitiveTag}
                    </div>
                </div>
                <div style="display:flex; gap:1.5rem; font-size:0.8rem; color:var(--text-muted); margin-bottom:0.6rem; flex-wrap:wrap;">
                    <span>📅 Breach date: <strong style="color:var(--text-secondary);">${b.breach_date}</strong></span>
                    <span>👥 Accounts affected: <strong style="color:var(--text-secondary);">${pwnFormatted}</strong></span>
                </div>
                ${b.description ? `<div style="font-size:0.82rem; color:var(--text-secondary); line-height:1.5; margin-bottom:0.6rem;">${b.description}</div>` : ''}
                <div style="font-size:0.78rem; color:var(--text-muted); margin-top:0.4rem;">
                    <strong>Exposed data:</strong><br>
                    <div style="margin-top:0.3rem;">${dataClasses}</div>
                </div>
            </div>`;
    });
